...
from app.module.user.presentation.dependancies import (
...
@router.get(
    "/{user_id}", response_model=UserResponseDto | None, status_code=status.HTTP_200_OK
)
def get_user(
...
@router.post(
    ...
    dependencies=[
        Depends(require_roles(UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.SUPERVISOR))
    ],
)
def create_user(
...
def delete_user(
    user_id: int, use_case: DeleteUserUseCase = Depends(get_delete_user_use_case)
) -> None:
    return use_case.execute(user_id)
